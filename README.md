...
│
├── ui/
│   ├── streamlit_view.py       # Streamlit 専用の描画・入力・RT計測・品質報告UI
...
LLMの性能によっては、まれに意図しない言語が混ざった問題（例: ロシア語の問題文に中国語が混入する）が生成されることがあります。そうした問題に遭遇したら、出題画面の **「⚠️ この問題はおかしい（言語混在・文章破損など）」** を開き、不備の種類を選んで「この問題を報告してスキップする」を押してください。

- 報告された問題はその場で `item_bank.json` の在庫から取り除かれ、`data/flagged_items.json` に理由・日時付きで退避されます。
- この操作は習熟度（mastery）や試行ログには影響しません（正誤としてカウントされません）。
- 在庫が減った分は、次回 `scripts/pre_generate.py` を実行した際に自動的に補充されます。セッション中に追加でLLMを呼ぶ必要はありません。

```bash
# 報告分を含めて在庫を補充する（通常の事前生成と同じコマンドでOK）
python scripts/pre_generate.py --concept-id ru_motion_verbs --target-stock 20
```

`data/flagged_items.json` を確認すれば、どんな問題がどんな理由で報告されたかを一覧できます。
...
- **「学習対象の概念がありません」と表示される**:
  ...
  すでに `pre_generate.py` を実行済みで `data/user_state.json` にも概念が入っているのにこのエラーが出続ける場合は、エラー画面の **「🔄 状態を再読込する」ボタン**を押してください（`state` / `item_bank` は出題選択のタイミングで毎回ディスクから読み直されるため、通常はサーバーを再起動しなくても反映されます）。
...
- **品質不良の問題が繰り返し出る**: 「⚠️ この問題はおかしい」で報告してスキップしたあと、`data/flagged_items.json` に同じパターンの報告が多く溜まっている場合は、`prompts/scenario_gen.txt` の指示文を見直すか、`llm.model` をより高性能なモデルに変更することを検討してください。
